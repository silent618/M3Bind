...
class MGCA(LightningModule):
    '''Pytorch lightning implementation of XRay_CLIP'''
    ...
    def forward(self, imgs, caption_ids, attention_mask, token_type_ids,
        ...
        center_embeds = self.text_encoder_q(center_input_ids, center_attention_mask, center_token_type_id)
        ...
        mask = torch.from_numpy(np.array(sents)[:, 1:] == "[PAD]").type_as(imgs).bool()
        ...
        if self.hparams.bidirectional:
            ...
            if "vit" not in self.hparams.img_encoder:
                ...
                patch_atten_weights = torch.ones(
                    bz, patch_num).type_as(imgs) / patch_num
        ...
        loss_total = (self.hparams.lambda_1 * loss_ita + self.hparams.lambda_2 * loss_local +
                      self.hparams.lambda_3 * loss_proto)

        return loss_total, img_emb_q, report_emb_q, center_embeds
...
def cli_main():
    ...
    ckpt_dir = os.path.join(
        BASE_DIR, f"../../../data/ckpts/XRay_CLIP/{extension}")
    ...
    wandb_logger = WandbLogger(
        project="XRay_CLIP", save_dir=logger_dir, name=extension)
